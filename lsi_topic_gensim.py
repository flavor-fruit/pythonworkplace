...
def dealWithLsi(documents,lsi,path):
    dictionary = corpora.Dictionary(documents)
    corpus = [dictionary.doc2bow(text) for text in documents]
    X= corpus2matrix(lsi[corpus])
    data = pd.DataFrame(X)
    data.to_csv(path)



def creatLSI(documents,topic_num):
    dictionary = corpora.Dictionary(documents)
    # print dictionary
    ...
    for i in range(0, topic_num):
       # print  get_topic_term(lda, i, len(dictionary))
        words = topic2words(lsi.show_topic(i, len(dictionary)))
...
if __name__ == '__main__':
    ...
    lsi = creatLSI(train_documents,10)

    dealWithLsi(train_documents,lsi,'train_topic.csv')
    ...
    dealWithLsi(test_documents,lsi,'test_topic.csv')
